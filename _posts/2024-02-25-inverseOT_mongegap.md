...
Since strong duality holds for entropic OT, one has the equality between the primal optimal objective and the dual optimal objective _ie_ \eqref{eq:eot} = \eqref{eq:dual_eot}.
...
:bulb: As discussed earlier, optimizing _w.r.t._ $$\mathbf{C}$$ an argmin like in \eqref{eq:inner_invot} requires computationally demanding tools such as unrolling or implicit function theorem. On the contrary, optimizing the min as in \eqref{eq:min_formulation_invot} is much simpler. It can be done using Danskin’s theorem (or envelope theorem).
...
for n in N_list: # Load n datapoints of the Swiss roll
